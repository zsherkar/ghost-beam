...
Ghost Beam is a local simulated MVP.

- No real accelerator hardware is connected.
- No EPICS reads or writes are performed.
- No network discovery, serial scanning, camera access, or instrument control is used.
...
The MVP demonstrates the platform architecture: a trust-and-memory layer that can sit between an autonomous optimizer/agent and a scientific facility after facility review.
